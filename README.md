...
4. Создайте в корне проекта файл `.env` и поместите в него ваши значения следующих переменных: `DATABASE_URL` (путь до БД, созданной выше. В формате postgresql://**user**:**password**@localhost:5432/**dbname**?schema=public"), где **user** - ваше имя в БД, **password** - ваш пароль в БД, **dbname** - имя БД, `AUTH_SECRET` (Команда `npx auth` для установки нужного пакета, затем `npx auth secret` сгенерирует автоматически данную переменную в `.env.local`, её перенесите в `.env`, `.env.local` удалите), `EMAIL_SENDER` (Вам нужно зарегистрироваться на сайте Resend, и ваша почта и будет этим значением, на неё будут приходить письма), `RESEND_API_KEY`(Вам нужно зарегистрироваться на сайте Resend, и там взять данное значение), `NEXT_PUBLIC_APP_URL="http://localhost:3000"`.
